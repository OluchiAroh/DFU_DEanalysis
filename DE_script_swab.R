...
file_path <- "Meta/Final_meta_swab_healvsamp.csv"
...
A = plotPCA(vsdata, intgroup="Batch", pcsToUse = 1:2)
B = plotPCA(vsdata, intgroup="Batch", pcsToUse = 3:4)

C = A + ggtitle('P1/P2') + labs(color = "Batch") + stat_ellipse()
D = B + ggtitle('P3/P4') + labs(color = "Batch") + stat_ellipse()

C + D + plot_annotation(title = "PCA plot for Batch - Swab") 
...
##Since I have fewer genes, I will only select significant nes based on the p-value
...
all_res_tb  <- all_res_sig_2%>% dplyr::mutate(genelabels = "")
...
ggplot(all_res_tb, aes(x = log2FoldChange, y = -log10(padj))) +
  ...
  geom_text_repel(aes(label = genelabels), box.padding = 0.5, max.overlaps = Inf) +
  ggtitle("HealedvsAmputated - Swab") +
  xlab("log2 fold change") + 
  ylab("-log10 adjusted p-value") +
  theme(legend.position = "none",
        plot.title = element_text(size = rel(1.5), hjust = 0.5),
